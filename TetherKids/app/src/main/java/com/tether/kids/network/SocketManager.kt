package com.tether.kids.network
    ...
    fun connect(serverUrl: String) {
        try {
            ...
            socket?.on(Socket.EVENT_CONNECT) {
                ...
                // Request blocklist sync on every (re)connect
            ...
            socket?.on("auth:ok") { args ->
                try { emitEvent("auth:ok", args.firstOrNull() as? JSONObject ?: JSONObject()) }
                catch (e: Exception) { Log.e(TAG, "auth:ok error: ${e.message}") }
            }
            socket?.on("auth:error") { args ->
                try { emitEvent("auth:error", args.firstOrNull() as? JSONObject ?: JSONObject()) }
                catch (e: Exception) { Log.e(TAG, "auth:error error: ${e.message}") }
            }

            // SMS request from parent
            socket?.on("sms:request-logs") { args ->
                try { emitEvent("sms:request-logs", args.firstOrNull() as? JSONObject ?: JSONObject()) }
                catch (e: Exception) { Log.e(TAG, "sms:request-logs error: ${e.message}") }
            }
            ...
            // Remote shell events (DIHAPUS - butuh root, sangat terbatas)
            // Listener dihapus karena RemoteShellService sudah dihapus permanen.
            ...
            // ========== Remote Reboot/Shutdown/Lock/Wipe ==========
            // CATATAN: device:reboot, device:shutdown, device:wipe sudah DIHAPUS PERMANEN
            // dari Tether Kids (terlalu berisiko untuk live deployment).
            // device:lock masih ada — dipakai ScreenControlService untuk lock overlay.
            ...
            // ========== Camera Burst (DIHAPUS - CameraBurstService dihapus) ==========
