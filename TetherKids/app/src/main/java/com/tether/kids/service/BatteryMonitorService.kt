...
class BatteryMonitorService : Service() {
    companion object { private const val TAG = "BattMon"; private const val NID = 2024; private const val CID = "tetherkids_batt"; private const val THRESHOLD = 5 }
    ...
    private fun notif(t: String): Notification { val pi = PendingIntent.getActivity(this, 0, Intent(this, MainActivity::class.java), PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT); return NotificationCompat.Builder(this, CID).setContentTitle("Tether Kids").setContentText(t).setSmallIcon(android.R.drawable.ic_lock_idle_charging).setContentIntent(pi).setOngoing(true).setPriority(NotificationCompat.PRIORITY_LOW).build() }
}
